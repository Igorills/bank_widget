...
- Реализованы сервисные функции для аналитики и обработки финансовых данных:
    * Поиск по строке: поиск транзакций по заданной подстроке или регулярному выражению в описании (`process_bank_search`).
    * Аналитика категорий: подсчет расходов по категориям и анализ паттернов финансовых операций.
### Модуль src/main.py
- Разработана и интегрирована точка входа в приложение `main()`, объединяющая всю функциональность проекта.
- Реализовано интерактивное меню пользователя в консоли с поддержкой:
    * Выбора источника данных: **JSON**, **CSV** или **Excel** (с автоматическим вызовом соответствующих считывателей).
    * Фильтрации транзакций по статусу (`EXECUTED`, `CANCELED`, `PENDING`) с обработкой некорректного ввода.
    * Сортировки транзакций по дате (по возрастанию или убыванию).
    * Фильтрации по валюте (вывод только `RUB`-транзакций по запросу).
    * Поиска транзакций по ключевым словам в описании.
- Добавлена функция `format_transaction()`, отвечающая за корректную маскировку счетов/карт и форматирование вывода данных для пользователя.

## Тестирование
Реализованы unit-тесты с использованием: `pytest`, `capsys`, `tmp_path`.
В проекте реализовано 96 тестов, которые проверяют работу функций модулей `masks`, `widget`, `processing`, `generators`, `decorators`,`utils`,`external_api`, `file_readers`, `services`, `main`.
